...
export class AppComponent {
  ...
  public results: any[] = [
    { title: 'testing', subtitle: 'testing sub', hint: 'testing' },
    { title: 'testing', subtitle: 'testing sub' },
  ];
  public arrowkeyLocation: number = 0;

  constructor(
    private electron: ElectronService,
    private UIService: ElectronUIService
  ) {
    this.RegisterShapeChanges();
    this.termChange.subscribe((term) => this.QueryContent(term));
    this.UIService.GetResultsStream().subscribe((x) => console.log(x));
  }
        ...
        switchMap((shouldExpand) => {
          if (shouldExpand) {
            if (this.electron.isElectronApp)
              this.electron.ipcRenderer.send(ElectronCommand.ResizeWindow, {
                height: 650,
              });
  ...
  correctSelection() {
    ...
    else if (this.arrowkeyLocation >= this.results.length)
  ...
  onSelect() {
    if (this.electron.isElectronApp)
      this.electron.ipcRenderer.send(
        ElectronCommand.OnSelect,
        this.results[this.arrowkeyLocation]
      );
    //this.term=""
  }
}
